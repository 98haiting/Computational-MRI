...
def sense_recon(aliased_imgs, sens_maps, PSI, R):
    ...
    for idx_PE, idx_RO in zip(*nonzero_idx):
        if not unaliased_img[idx_PE, idx_RO]:
            # SENSE
            locs = np.mod(np.arange(idx_PE, PE + idx_PE, PE / R, dtype=int), PE)
            C = sens_maps[locs, idx_RO, :].T
            C_pinv = pinv(C)
            idx_PE_alias = get_alias_idx(PE, R, locs)
            unaliased_img[locs, idx_RO] = C_pinv @ aliased_imgs[idx_PE_alias, idx_RO, :]

    return unaliased_img 