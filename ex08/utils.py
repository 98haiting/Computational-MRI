...
def create_gif(recons: list, title: str, duration: int = 100):
    ...
    recon_gif[0].save(f'Results/{title}', format="GIF", append_images=recon_gif[1:], save_all=True, duration=duration, loop=0)
...
def dwt2(x, wavelet='db4', mode='periodization', level=4, axes=(-2, -1)):
    ...
    Returns:
        Complex wavelet transformed data (coeff_arr, coeff_slice)
            - coeff_arr: Approximated coefficients
            - coeff_slice: Coefficient details
    """
    ...
    coeff_arr = c_real_array + 1j * c_imag_array

    return coeff_arr, coeff_slice


def idwt2(coeff_arr, coeff_slice, wavelet='db4', mode='periodization'):
    """Inverse discrete 2D wavelet transform

    Args:
        coeff_arr (ndarray):        Approximated coefficients
        coeff_slice (list):         Coefficient details
        wavelet (str, optional):    wavelet to use. Defaults to 'db4'.
        mode (str, optional):       Signal extension mode. Defaults to 'periodization'.
    ...
    """
    c_real_thresh = pywt.array_to_coeffs(np.real(coeff_arr), coeff_slice, output_format='wavedec2')
    c_imag_thresh = pywt.array_to_coeffs(np.imag(coeff_arr), coeff_slice, output_format='wavedec2')
